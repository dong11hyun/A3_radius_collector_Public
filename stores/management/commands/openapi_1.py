"""
서울시 휴게음식점 인허가 정보 수집 (편의점만)
- Open API에서 페이지네이션으로 전체 데이터 수집
- 편의점 + 영업중인 것만 필터링하여 PostgreSQL에 저장
...
from .gu_codes import get_restaurant_service, list_supported_gu
...
class Command(BaseCommand):
    help = '서울시 편의점 인허가 정보 수집 (--gu 옵션으로 대상 구 지정)'
    ...
    def handle(self, *args, **options):
        ...
        # 서비스명 동적 조회
        try:
            service_name = get_restaurant_service(target_gu)
